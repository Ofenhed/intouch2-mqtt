...
use palette::{Yxy,Srgb,convert::FromColorUnclamped};
...
impl DeconzClient {
  pub fn make_deconz(deconz_host: String, api_key: String, group_name: String, dark_group_name: String) -> Self {
    DeconzClient {
        api_url: ["http://", &deconz_host, "/api/", &api_key, "/"].concat(),
        group_name: group_name,
        dark_group_name: dark_group_name,
        cache: None,
    }
  }
  pub async fn update_cache(&mut self) -> Result<&DeconzCache, &'static str> {
    let (group_api_url, dark_group_api_url) = async {
      ...
      let response = client.get(&[&self.api_url, "groups"].concat()).send().await.unwrap();
      let groups = from_str(&response.text().await.unwrap_or("{}".to_string())).unwrap_or(json!(null));
      let empty_map = Map::new();
      let groups_object = groups.as_object().unwrap_or(&empty_map).iter();
      let (light, dark) = groups_object.fold((None, None), |(light, dark), (key, group)| { if group["name"].as_str() == Some(&self.group_name) { (Some(key), dark) }
                                                                                         else if group["name"].as_str() == Some(&self.dark_group_name) { (light, Some(key)) }
                                                                                         else { (light, dark) } });
      println!("Found light groups {:?} and {:?}", light, dark);
      match (light, dark) {
        (Some(light), Some(dark)) => Ok(([&self.api_url, "groups/", light].concat(), [&self.api_url, "groups/", dark].concat())),
  ...
  async fn push_status(&mut self, push_values: &PushStatusList) -> Result<(), &'static str> {
    ...
    if let (Some((red, green, blue, intencity)), _) = get_status_rgba(push_values) {
      let xy = Yxy::from_color_unclamped(Srgb::new(red as f32 / std::u8::MAX as f32,
    ...
    if let Some(request_object) = request_object {
      ...
      let response = client.put(&[&current_group_api_url, "/action"].concat()).body(request_object.to_string()).send().await;
      println!("{:?}", response);
      if let Ok(response) = response {
        let _ = println!("Response for matching light {}: {}", response.status(), response.text().await.unwrap_or("NO RESPONSE RECEIVER".to_string()));
      }
    };
    if let Some(dark_request_object) = dark_request_object {
      println!("Sending command {}", dark_request_object.to_string());
      let response = client.put(&[&current_dark_group_api_url, "/action"].concat()).body(dark_request_object.to_string()).send().await;
      println!("{:?}", response);
      if let Ok(response) = response {
        println!("Response for dark light {}: {}", response.status(), response.text().await.unwrap_or("NO RESPONSE RECEIVER".to_string()));
      }
    };
    Ok(())
  }
...
async fn main() -> Result<(), std::io::Error> {
  ...
  let mut deconz_client = DeconzClient::make_deconz(deconz_host, api_key, deconz_group_name, deconz_dark_group_name);
        ...
        if let Ok(len) = socket.recv(&mut buf) {
          match parse_network_data(&buf[0..len]) {
            ...
            Ok(([], NetworkPackage::Authorized{src: x, dst: y, data: NetworkPackageData::PushStatus(data)})) => {
              ...
              deconz_client.push_status(&data).await.map_err(|e| std::io::Error::new(std::io::ErrorKind::Other, format!("Push status error: {}", e)))?;
