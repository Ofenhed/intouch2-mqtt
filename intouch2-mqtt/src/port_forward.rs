...
impl PortForward {
  pub async fn run(self) -> Result<(), PortForwardError> {
        ...
        'ignore_package: loop {
          match timeout_at(timeout, sock_spa.recv(buf.as_mut())).await {
            ...
            Ok(len) => match parse_network_data(&buf[0..len?]) {
              Err(_) | Ok(NetworkPackage::Addressed { .. }) => continue 'ignore_package,
              Ok(NetworkPackage::Hello(spa_name)) => break 'retry Ok(spa_name.into_owned()),
            },
          }
        }
        if tries == 0 {
    ...
    let mut hello_response = Arc::new(RwLock::new(compose_network_data(&NetworkPackage::Hello(
    ...
    loop {
      while let Some(job) = workers.join_next().await {
        ...
        match &mut job_result {
          ...
          SocketData::FromSpa { recv_sock, .. } | SocketData::SpawnSpaListener { recv_sock } => {
            let mut buf = buffers.take_or(|| Vec::with_capacity(NET_BUFFER_SIZE));
            let Some(recv_sock) = std::mem::take(recv_sock) else {
              unreachable!(
        ...
        match job_result {
          ...
          SocketData::FromSpa { data, .. } => match parse_network_data(&data) {
            ...
            Ok(NetworkPackage::Hello(id)) => {
              if id[..] != spa_id[..] {
                if self.verbose {
                  eprintln!("Spa changed name to {}", String::from_utf8_lossy(&id));
                }
                spa_id = id.into();
                spa_hello = SpaHello::new(&spa_id)?;
                *hello_response.write().await =
                  compose_network_data(&NetworkPackage::Hello(Cow::Borrowed(&spa_id)));
              }
            }
            Ok(NetworkPackage::Addressed { dst: None, .. }) => {
              if self.verbose {
          ...
          SocketData::SpawnClientListener { .. } | SocketData::SpawnSpaListener { .. } => (),
          SocketData::SendCompleted { .. } => unreachable!("SendCompleted is filtered out above"),
        }
      }
    }
    // let spa_pipe = {
