...
fn parse_hello() {
  assert_eq!(
    ...
    Ok((EMPTY, NetworkPackage::Hello(b"1".to_vec())))
  );
}
  ...
  assert_eq!(
    ...
    Ok((
      ...
      NetworkPackage::Authorized {
        src: None,
        dst: None,
        data: NetworkPackageData::Unknown(b"APUNG".to_vec())
      }
    ))
  )
}
...
fn id_packets() {
  let packets = vec![
    NetworkPackage::Hello(b"My hello".to_vec()),
    NetworkPackage::Authorized {
      src: Some(b"some-src".to_vec()),
      dst: None,
      data: NetworkPackageData::Ping,
    },
    NetworkPackage::Authorized {
      src: Some(b"some-src".to_vec()),
      dst: Some(b"some-dest".to_vec()),
      data: NetworkPackageData::Pong,
    },
    NetworkPackage::Authorized {
      src: None,
    ...
    NetworkPackage::Authorized {
      src: None,
      dst: None,
      data: NetworkPackageData::Version(b"some_version".to_vec()),
    },
